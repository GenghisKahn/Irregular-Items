...
import com.genghiskahn1992.irregularitems.containers.CombustionGeneratorContainer;
...
public class CombustionGeneratorTile extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
    ...
    public CombustionGeneratorTile(){
        super(ModBlocks.COMBUSTIONGENERATOR_TILE);
    }
    ...
    public void tick() {
        ...
        if(counter > 0) {
            counter--;
            energy.ifPresent(e -> ((CustomEnergyStorage)e).addEnergy(Config.COMBUSTIONGENERATOR_GENERATE.get()));
            markDirty();
        }
        ...
        if (counter <=0) {
            handler.ifPresent(h -> {
                ...
                if(FurnaceTileEntity.getBurnTimes().keySet().contains(stack.getItem()) &&
                    ...
                    counter = Math.round(Config.COMBUSTIONGENERATOR_TICKS.get() * FurnaceTileEntity.getBurnTimes().get(stack.getItem()));
                    markDirty();
                }
            });
        }
    ...
    private IEnergyStorage createEnergy() {
        return new CustomEnergyStorage(Config.COMBUSTIONGENERATOR_MAXPOWER.get(), 0);
    }
                    ...
                    if (te != null){
                        boolean doContinue = te.getCapability(CapabilityEnergy.ENERGY, direction).map(handler -> {
                            if (handler.canReceive()){
                                int received = handler.receiveEnergy(Math.min(capacity.get(), Config.COMBUSTIONGENERATOR_SEND.get()), false);
